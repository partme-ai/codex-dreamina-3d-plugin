...
class SecretPatternTests(unittest.TestCase):
    # Shipped content only. `__pycache__` is generated bytecode that embeds
    # source literals, and `companions/` is a CI checkout of other repos.
    SCAN_EXCLUDED = {".git", "__pycache__", "companions"}

    def test_no_secrets_anywhere_in_repo(self) -> None:
        for path in ROOT.rglob("*"):
            if not path.is_file() or self.SCAN_EXCLUDED & set(path.parts):
                continue
...
class CompanionCheckoutScanTests(unittest.TestCase):
    """CI checks pinned companion repositories into a `companions/` directory
    inside this repo. Their fixtures are other repositories' content, validated
    by their own validators, so this plugin's secret scan must not fail on
    them. A real secret in our own tree must still be caught."""

    def _run_validator_on(self, mutate) -> list[str]:
        import shutil
        import tempfile
    ...
    def _plant(path: Path) -> None:
        ...
        path.write_text('KEY = "AIza' + "x" * 30 + '"\n')

    def test_companion_checkout_fixtures_are_not_scanned(self) -> None:
        errors = self._run_validator_on(
            lambda clone: self._plant(
                clone / "companions" / "codex-dreamina-design-plugin" / "tests" / "test_distribution_v7.py"
            )
        )
        self.assertEqual(errors, [], errors)

    def test_real_secret_in_our_own_tree_is_still_caught(self) -> None:
        errors = self._run_validator_on(lambda clone: self._plant(clone / "scripts" / "leaked.py"))
        self.assertTrue(
            any("secret-like content" in e and "leaked.py" in e for e in errors),
            errors,
        )
